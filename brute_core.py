...
class SimpleBruteForcer:
    ...
    def toggle_pause(self):
        self.pause.set() if not self.pause.is_set() else self.pause.clear()
    ...
    def attempt(self, pw):
        ...
        try:
            ...
            self.log(f"[ERR] {pw} -> request failed: {e}")
            return False

        classification = self.classify_response(r)
        self.log(f"[RESP] {pw} -> {classification}")

        if r.status_code == 200 and (
            "dashboard" in r.text or "/welcome" in r.url
        ):
            self.log(f"[HIT] {pw}")
    ...
    def worker(self):
        while (
            not self.passwords.empty()
            and not self.success.is_set()
    ...
    def run(self):
        ...
        if not self.success.is_set():
            self.log("❌ No password found")
